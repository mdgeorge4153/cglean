...
structure, and the proofs feel more laborious than the mathematics warrants.

The construction has a distinctly linear-algebraic character. $A[\sqrt{n}]$ is a
free $A$-module of rank two; multiplication is $A$-bilinear and so is determined
by its values on a basis; conjugation is an $A$-linear involution fixing $1$ and
negating $\sqrt{n}$; and the quantity $x \cdot \overline{x}$, which the ordering
and the inverse both depend on, behaves exactly like a determinant --- it is
multiplicative, and it vanishes only at zero.

This suggests an alternative development in which $A[\sqrt{n}]$ is presented as
$A[x]/(x^2 - n)$, and its properties are inherited from general facts about
quotients of polynomial rings rather than verified by hand. Mathlib has the
machinery for this, and the relationship is already recorded as
Lemma~\ref{thm:mathlibAdjoin}, currently unproved. The \code{Algebra} instance
this development registers is the beginning of such an account but is not yet
used for anything.

It is not obvious that this would be simpler overall. The elementary
presentation is what makes the sign computation transparent, and that
computation is the part this library actually needs; a quotient presentation may
well make the ordering harder to reason about even as it makes the ring
structure easier. It seems worth attempting on a branch to find out.
